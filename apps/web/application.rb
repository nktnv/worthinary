...
require_relative './views/webpack_helpers'
  ...
  class Application < Hanami::Application
    configure do
      ...
      security.content_security_policy %{
        ...
        base-uri 'self';
        default-src 'none';
        script-src 'self' 'unsafe-eval' http://localhost:3500;
        connect-src 'self' ws://localhost:3500 http://localhost:3500;
        img-src 'self' https: data: http://localhost:3500;
        style-src 'self' 'unsafe-inline' https:;
        font-src 'self' data: http://localhost:3500;
        object-src 'none';
